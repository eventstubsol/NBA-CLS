                ...
                <div class="tab-pane {{ $i === 1 ? "active show" : "" }}" id="sch-{{ $i }}">
                        ...
                        <ul class="nav nav-pills navtab-bg nav-justified" style="margin: 0px -5px;">
                            @foreach($master_rooms as $master_room => $rooms)
                                ...
                                @endphp
                                    <li class="nav-item">
                                        <a href="#sch-{{ $i }}-{{ $j }}" data-toggle="tab" aria-expanded="{{ $j === 1 ? 'true' : 'false' }}" class="nav-link @if($j === 1) active @endif">{{ $master_room  === 'peek_behind_corporate_veil' ? 'PEEK BEHIND THE CORPORATE VEIL' :   strtoupper(str_replace("_"," ", $master_room==='Auditorium' || $master_room==='auditorium' ? 'IMPACT AUDITORIUM' : $master_room ))}}</a>
                                    </li>
                            @endforeach
                        </ul>
                                                                ...
                                                                <div class="avatar-group mb-2">
                                                                    ...
                                                                    @foreach($event['speakers'] as $speaker)
                                                                        ...
                                                                        data-original-title="{{ isset($speaker->speaker->name) ?  $speaker->speaker->name .' '. $speaker->speaker->last_name: "" }}">
                                                                            ...
                                                                            <img src="{{ isset($speaker->speaker) ? $speaker->speaker->profileImage ? assetUrl($speaker->speaker->profileImage) : "https://congress2021web.fra1.digitaloceanspaces.com/uploads/default-profile.jpeg" : "" }}"
